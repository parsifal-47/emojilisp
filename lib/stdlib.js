module.exports = (function() {
	...
	function defun(context, AST) {
		if (AST.contents.length !== 3) 
			throw new Error("You need exactly three elements in a list to define function");
	...
	function lambda(context, AST) {
		if (AST.contents.length !== 2)
			throw new Error("You need exactly two elements in a list to define lambda");
	...
	function iff(context, AST) {
		if (AST.contents.length !== 3) 
			throw new Error("You need exactly three elements (condition, then, else)");
	...
	function morethan(context, AST) {
		if (AST.contents.length !== 2)
			throw new Error("You need exactly two elements to compare");
	...
	function minus(context, AST) {
		if (AST.contents.length !== 2)
			throw new Error("You need exactly two elements for minus");
		return new literal(context.number(context.execute(AST.contents[0])) - 
	...
	function plus(context, AST) {
		if (AST.contents.length !== 2)
			throw new Error("You need exactly two elements for plus");
		return new literal(context.number(context.execute(AST.contents[0])) +
				context.number(context.execute(AST.contents[1])));
	}
	...
	function cdr(context, AST) {
		if (AST.contents.length !== 1)
			throw new Error("You need exactly one element for cdr");
		var rl = context.execute(AST.contents[0]);
		if (rl.type !== "List")
			throw new Error("Cdr argument should be list, but " + rl.type + " is given");

		rl.contents.splice(0,1);
		return rl;
	}
